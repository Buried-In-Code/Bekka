...
public class TestIssue
{
    ...
    [Test(Description = "Test using the GetIssue function with a valid Id")]
    public async Task TestGetIssue()
    {
        var result = await _metron.GetIssue(id: 1088);
    ...
    [Test(Description = "Test using the GetIssue function with an invalid Id")]
    public void TestGetIssueFail()
    {
        Assert.ThrowsAsync<ServiceException>(async () => await _metron.GetIssue(id: -1));
    }

    [Test(Description = "Test using the ListIssues function with a valid search")]
    public async Task TestListIssues()
    {
        var results = await _metron.ListIssues(parameters: new Dictionary<string, string> { { "series_id", 119.ToString() }, { "number", "1" } });
    ...
    [Test(Description = "Test using the ListIssues function with an invalid search")]
    public async Task TestListIssuesFail()
    {
        var results = await _metron.ListIssues(parameters: new Dictionary<string, string> { { "series_id", 119.ToString() }, { "number", "INVALID" } });
        Assert.That(results, Is.Empty);
    }

    [Test(Description = "Test using the GetIssueByComicvine function with a valid Id")]
    public async Task TestGetIssueByComicvine()
    {
        var result = await _metron.GetIssueByComicvine(comicvineId: 34352);
        Assert.That(result, Is.Not.Null);
        Assert.Multiple(() =>
        {
            Assert.That(result.CoverHash, Is.EqualTo("87386cc738ac7b38"));
    ...
    [Test(Description = "Test using the GetIssueByComicvine function with an invalid Id")]
    public void TestGetIssueByComicvineFail()
    {
        Assert.ThrowsAsync<ServiceException>(async () => await _metron.GetIssueByComicvine(comicvineId: -1));
    }
}